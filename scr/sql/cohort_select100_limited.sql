CREATE MATERIALIZED VIEW hypoxemia_cohort100_limited AS WITH tmp AS (
         ...
         SELECT tlmt.subject_id,
           ...
           FROM tlmt
          WHERE ((tlmt.ratio_pre_min >= (1)::numeric) AND (tlmt.ratio_pre_max <= (2)::numeric) AND ((tlmt.ratio_post_min >= (0)::numeric) AND (tlmt.ratio_post_max <= (1)::numeric)))
        ), oi_mv AS (
