...
CREATE MATERIALIZED VIEW hypoxemia_var AS
WITH hypoxemia_chart AS (
         SELECT chartevents.subject_id,
            ...
            max(
                CASE
                    WHEN (chartevents.itemid = ANY (ARRAY[1096, 505, 506, 60, 437, 220339, 224700, 686])) THEN chartevents.valuenum
                    ELSE NULL::double precision
                END) AS peep,
            max(
                CASE
                    WHEN (chartevents.itemid = ANY (ARRAY[1250, 118, 1103, 113, 220074])) THEN chartevents.valuenum
                    ELSE NULL::double precision
                END) AS cvp,
            max(
                CASE
                    WHEN (chartevents.itemid = ANY (ARRAY[8368, 8440, 8441, 8555, 220180, 220051, 118])) THEN chartevents.valuenum
                    ELSE NULL::double precision
                END) AS dbp,
           max(
                CASE
                    WHEN (chartevents.itemid = ANY (ARRAY[211, 220045])
                          AND (chartevents.valuenum > (0)::double precision)
                          AND (chartevents.valuenum < (300)::double precision) ) THEN chartevents.valuenum
                    ELSE NULL::double precision
                END) AS heartrate
           FROM mimiciii.chartevents
          WHERE ((chartevents.itemid = ANY (ARRAY[3785, 3837, 490, 779, 223835, 3420, 3422, 190, 1096, 505, 506, 60, 437, 220339, 224700, 686, 1250, 118, 1103, 113, 220074, 8368, 8440, 8441, 8555, 220180, 220051, 118, 211, 220045])) AND (chartevents.value IS NOT NULL))
 ...
 SELECT hypoxemia_chart.subject_id,
    ...
    NULL::double precision AS pao2,
    hypoxemia_chart.fio2,
    hypoxemia_chart.peep,
    hypoxemia_chart.cvp,
    hypoxemia_chart.dbp,
    hypoxemia_chart.heartrate
